...
def preprocess_qwen2(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            conv.append_message(role, sentence["value"])
        conversations.append(conv.get_prompt())
        if "<|im_end|>\n<|im_start|>assistant" in conversations[0]:
            conversations = [text.replace("<|im_end|>\n<|im_start|>assistant",LEARNABLE_TOKEN+" <|im_end|>\n<|im_start|>assistant") for text in conversations]
        else:
            raise ValueError("learnable token add failure.")
    ...
    for conversation, target in zip(conversations, targets): # process each sample
        ...
        rounds = conversation.split(conv.sep2)
        round_0 = rounds[0] + conv.sep2 #system message
        rounds = rounds[1:-1]
